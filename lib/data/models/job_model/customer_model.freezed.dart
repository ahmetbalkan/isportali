// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'customer_model.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

CustomerModel _$CustomerModelFromJson(Map<String, dynamic> json) {
  return _CustomerModel.fromJson(json);
}

/// @nodoc
mixin _$CustomerModel {
  @JsonKey(name: 'CustomerID')
  int? get customerID => throw _privateConstructorUsedError;
  @JsonKey(name: 'WebAddress')
  String? get webAddress => throw _privateConstructorUsedError;
  @JsonKey(name: 'CompanyName')
  String? get companyName => throw _privateConstructorUsedError;
  @JsonKey(name: 'InfoHtml')
  String? get infoHtml => throw _privateConstructorUsedError;
  @JsonKey(name: 'LogoPath')
  String? get logoPath => throw _privateConstructorUsedError;
  @JsonKey(name: 'IsUserSubscribedToCompany')
  bool? get isUserSubscribedToCompany => throw _privateConstructorUsedError;
  @JsonKey(name: 'CityName')
  String? get cityName => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $CustomerModelCopyWith<CustomerModel> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $CustomerModelCopyWith<$Res> {
  factory $CustomerModelCopyWith(
          CustomerModel value, $Res Function(CustomerModel) then) =
      _$CustomerModelCopyWithImpl<$Res, CustomerModel>;
  @useResult
  $Res call(
      {@JsonKey(name: 'CustomerID')
          int? customerID,
      @JsonKey(name: 'WebAddress')
          String? webAddress,
      @JsonKey(name: 'CompanyName')
          String? companyName,
      @JsonKey(name: 'InfoHtml')
          String? infoHtml,
      @JsonKey(name: 'LogoPath')
          String? logoPath,
      @JsonKey(name: 'IsUserSubscribedToCompany')
          bool? isUserSubscribedToCompany,
      @JsonKey(name: 'CityName')
          String? cityName});
}

/// @nodoc
class _$CustomerModelCopyWithImpl<$Res, $Val extends CustomerModel>
    implements $CustomerModelCopyWith<$Res> {
  _$CustomerModelCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? customerID = freezed,
    Object? webAddress = freezed,
    Object? companyName = freezed,
    Object? infoHtml = freezed,
    Object? logoPath = freezed,
    Object? isUserSubscribedToCompany = freezed,
    Object? cityName = freezed,
  }) {
    return _then(_value.copyWith(
      customerID: freezed == customerID
          ? _value.customerID
          : customerID // ignore: cast_nullable_to_non_nullable
              as int?,
      webAddress: freezed == webAddress
          ? _value.webAddress
          : webAddress // ignore: cast_nullable_to_non_nullable
              as String?,
      companyName: freezed == companyName
          ? _value.companyName
          : companyName // ignore: cast_nullable_to_non_nullable
              as String?,
      infoHtml: freezed == infoHtml
          ? _value.infoHtml
          : infoHtml // ignore: cast_nullable_to_non_nullable
              as String?,
      logoPath: freezed == logoPath
          ? _value.logoPath
          : logoPath // ignore: cast_nullable_to_non_nullable
              as String?,
      isUserSubscribedToCompany: freezed == isUserSubscribedToCompany
          ? _value.isUserSubscribedToCompany
          : isUserSubscribedToCompany // ignore: cast_nullable_to_non_nullable
              as bool?,
      cityName: freezed == cityName
          ? _value.cityName
          : cityName // ignore: cast_nullable_to_non_nullable
              as String?,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$_CustomerModelCopyWith<$Res>
    implements $CustomerModelCopyWith<$Res> {
  factory _$$_CustomerModelCopyWith(
          _$_CustomerModel value, $Res Function(_$_CustomerModel) then) =
      __$$_CustomerModelCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {@JsonKey(name: 'CustomerID')
          int? customerID,
      @JsonKey(name: 'WebAddress')
          String? webAddress,
      @JsonKey(name: 'CompanyName')
          String? companyName,
      @JsonKey(name: 'InfoHtml')
          String? infoHtml,
      @JsonKey(name: 'LogoPath')
          String? logoPath,
      @JsonKey(name: 'IsUserSubscribedToCompany')
          bool? isUserSubscribedToCompany,
      @JsonKey(name: 'CityName')
          String? cityName});
}

/// @nodoc
class __$$_CustomerModelCopyWithImpl<$Res>
    extends _$CustomerModelCopyWithImpl<$Res, _$_CustomerModel>
    implements _$$_CustomerModelCopyWith<$Res> {
  __$$_CustomerModelCopyWithImpl(
      _$_CustomerModel _value, $Res Function(_$_CustomerModel) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? customerID = freezed,
    Object? webAddress = freezed,
    Object? companyName = freezed,
    Object? infoHtml = freezed,
    Object? logoPath = freezed,
    Object? isUserSubscribedToCompany = freezed,
    Object? cityName = freezed,
  }) {
    return _then(_$_CustomerModel(
      customerID: freezed == customerID
          ? _value.customerID
          : customerID // ignore: cast_nullable_to_non_nullable
              as int?,
      webAddress: freezed == webAddress
          ? _value.webAddress
          : webAddress // ignore: cast_nullable_to_non_nullable
              as String?,
      companyName: freezed == companyName
          ? _value.companyName
          : companyName // ignore: cast_nullable_to_non_nullable
              as String?,
      infoHtml: freezed == infoHtml
          ? _value.infoHtml
          : infoHtml // ignore: cast_nullable_to_non_nullable
              as String?,
      logoPath: freezed == logoPath
          ? _value.logoPath
          : logoPath // ignore: cast_nullable_to_non_nullable
              as String?,
      isUserSubscribedToCompany: freezed == isUserSubscribedToCompany
          ? _value.isUserSubscribedToCompany
          : isUserSubscribedToCompany // ignore: cast_nullable_to_non_nullable
              as bool?,
      cityName: freezed == cityName
          ? _value.cityName
          : cityName // ignore: cast_nullable_to_non_nullable
              as String?,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$_CustomerModel implements _CustomerModel {
  _$_CustomerModel(
      {@JsonKey(name: 'CustomerID')
          this.customerID,
      @JsonKey(name: 'WebAddress')
          this.webAddress,
      @JsonKey(name: 'CompanyName')
          this.companyName,
      @JsonKey(name: 'InfoHtml')
          this.infoHtml,
      @JsonKey(name: 'LogoPath')
          this.logoPath,
      @JsonKey(name: 'IsUserSubscribedToCompany')
          this.isUserSubscribedToCompany,
      @JsonKey(name: 'CityName')
          this.cityName});

  factory _$_CustomerModel.fromJson(Map<String, dynamic> json) =>
      _$$_CustomerModelFromJson(json);

  @override
  @JsonKey(name: 'CustomerID')
  final int? customerID;
  @override
  @JsonKey(name: 'WebAddress')
  final String? webAddress;
  @override
  @JsonKey(name: 'CompanyName')
  final String? companyName;
  @override
  @JsonKey(name: 'InfoHtml')
  final String? infoHtml;
  @override
  @JsonKey(name: 'LogoPath')
  final String? logoPath;
  @override
  @JsonKey(name: 'IsUserSubscribedToCompany')
  final bool? isUserSubscribedToCompany;
  @override
  @JsonKey(name: 'CityName')
  final String? cityName;

  @override
  String toString() {
    return 'CustomerModel(customerID: $customerID, webAddress: $webAddress, companyName: $companyName, infoHtml: $infoHtml, logoPath: $logoPath, isUserSubscribedToCompany: $isUserSubscribedToCompany, cityName: $cityName)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$_CustomerModel &&
            (identical(other.customerID, customerID) ||
                other.customerID == customerID) &&
            (identical(other.webAddress, webAddress) ||
                other.webAddress == webAddress) &&
            (identical(other.companyName, companyName) ||
                other.companyName == companyName) &&
            (identical(other.infoHtml, infoHtml) ||
                other.infoHtml == infoHtml) &&
            (identical(other.logoPath, logoPath) ||
                other.logoPath == logoPath) &&
            (identical(other.isUserSubscribedToCompany,
                    isUserSubscribedToCompany) ||
                other.isUserSubscribedToCompany == isUserSubscribedToCompany) &&
            (identical(other.cityName, cityName) ||
                other.cityName == cityName));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(runtimeType, customerID, webAddress,
      companyName, infoHtml, logoPath, isUserSubscribedToCompany, cityName);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$_CustomerModelCopyWith<_$_CustomerModel> get copyWith =>
      __$$_CustomerModelCopyWithImpl<_$_CustomerModel>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$_CustomerModelToJson(
      this,
    );
  }
}

abstract class _CustomerModel implements CustomerModel {
  factory _CustomerModel(
      {@JsonKey(name: 'CustomerID')
          final int? customerID,
      @JsonKey(name: 'WebAddress')
          final String? webAddress,
      @JsonKey(name: 'CompanyName')
          final String? companyName,
      @JsonKey(name: 'InfoHtml')
          final String? infoHtml,
      @JsonKey(name: 'LogoPath')
          final String? logoPath,
      @JsonKey(name: 'IsUserSubscribedToCompany')
          final bool? isUserSubscribedToCompany,
      @JsonKey(name: 'CityName')
          final String? cityName}) = _$_CustomerModel;

  factory _CustomerModel.fromJson(Map<String, dynamic> json) =
      _$_CustomerModel.fromJson;

  @override
  @JsonKey(name: 'CustomerID')
  int? get customerID;
  @override
  @JsonKey(name: 'WebAddress')
  String? get webAddress;
  @override
  @JsonKey(name: 'CompanyName')
  String? get companyName;
  @override
  @JsonKey(name: 'InfoHtml')
  String? get infoHtml;
  @override
  @JsonKey(name: 'LogoPath')
  String? get logoPath;
  @override
  @JsonKey(name: 'IsUserSubscribedToCompany')
  bool? get isUserSubscribedToCompany;
  @override
  @JsonKey(name: 'CityName')
  String? get cityName;
  @override
  @JsonKey(ignore: true)
  _$$_CustomerModelCopyWith<_$_CustomerModel> get copyWith =>
      throw _privateConstructorUsedError;
}
